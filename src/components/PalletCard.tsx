        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 space-y-2">
            <div className="flex items-center gap-3">
              {pallet.grade && (
                <span className="text-lg font-semibold text-foreground uppercase px-3 py-1.5 bg-muted rounded">
                  {pallet.grade}
                </span>
              )}
              <p className="text-xl font-medium text-foreground uppercase flex-1">
                {getDisplayDescription()}
              </p>
            </div>
            <h3 className="text-3xl font-bold text-primary uppercase">{pallet.pallet_number}</h3>
          </div>
          {!isHistory && (
      ...
      <CardFooter className="flex gap-2 p-5 pt-0">
        {!isHistory ? (
          <Button
            ...
            className="h-9 px-4 text-sm text-accent border-accent hover:bg-accent hover:text-accent-foreground"
          >
